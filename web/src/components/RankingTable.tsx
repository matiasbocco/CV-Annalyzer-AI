...
export default function RankingTable({ ranking }: { ranking: Candidate[] }) {
  ...
  return (
    <div className="bg-[#111118] border border-slate-800 rounded-xl overflow-x-auto">
      <table className="w-full text-sm">
        <thead>
          <tr className="bg-slate-800/50 border-b border-slate-800 text-left">
            <th className="px-3 sm:px-4 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wide w-10">#</th>
            <th className="px-3 sm:px-4 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wide">{t.rankCandidate}</th>
            <th className="px-3 sm:px-4 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wide w-24 sm:w-32">{t.rankScore}</th>
            <th className="px-3 sm:px-4 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wide w-24">{t.rankLevel}</th>
            <th className="hidden sm:table-cell px-4 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wide w-28">{t.rankSource}</th>
          </tr>
        </thead>
        <tbody>
          {ranking.map((c, i) => (
            <tr key={c.filename} className="border-b border-slate-800/60 last:border-0 hover:bg-slate-800/30 transition-colors">
              <td className="px-3 sm:px-4 py-3 text-slate-500 font-medium">{i + 1}</td>
